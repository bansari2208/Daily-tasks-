...
def calculate_monthly_cost(req_cost):
    return req_cost * MONTHLY_TICKETS


def calculate_savings(old_cost, new_cost):
    return old_cost - new_cost


def calculate_savings_percent(old_cost, new_cost):
...
def print_model_comparison_table():
    ...
    for model_name in MODEL_PRICING:
        ...
        monthly_cost = calculate_monthly_cost(req_cost)
...
def print_prompt_comparison_table():
    ...
    cost_a = calculate_monthly_cost(calculate_request_cost(p_a["input_tokens"], p_a["output_tokens"], "GPT-4.1"))
    cost_b = calculate_monthly_cost(calculate_request_cost(p_b["input_tokens"], p_b["output_tokens"], "GPT-4.1"))
    diff = calculate_savings(cost_a, cost_b)
    pct_savings = calculate_savings_percent(cost_a, cost_b)
...
def print_language_inflation_analysis():
    ...
    hindi_tokens = len(hindi_text.split())
...
def print_prompt_caching_simulation() -> tuple[float, float]:
    ...
    savings = calculate_savings(original_cost, cached_total_cost)
    pct_saved = calculate_savings_percent(original_cost, cached_total_cost)
...
def print_batch_api_simulation() -> tuple[float, float]:
    ...
    p_b = PROMPT_SPECS["Prompt B (Optimized)"]
    normal_cost = calculate_monthly_cost(calculate_request_cost(p_b["input_tokens"], p_b["output_tokens"], "GPT-4.1 Nano"))
    batch_cost = normal_cost * 0.50
    savings = calculate_savings(normal_cost, batch_cost)
    pct_saved = calculate_savings_percent(normal_cost, batch_cost)
...
def print_comprehensive_comparison_table():
    ...
    naive_cost = calculate_monthly_cost(calculate_request_cost(p_a["input_tokens"], p_a["output_tokens"], "GPT-4.1"))
    opt_prompt_cost = calculate_monthly_cost(calculate_request_cost(p_b["input_tokens"], p_b["output_tokens"], "GPT-4.1"))
    cheaper_model_cost = calculate_monthly_cost(calculate_request_cost(p_a["input_tokens"], p_a["output_tokens"], "GPT-4.1 Mini"))

    # GPT-4.1 Nano uncached
    nano_base = calculate_monthly_cost(calculate_request_cost(p_b["input_tokens"], p_b["output_tokens"], "GPT-4.1 Nano"))
    ...
    for name, model, cost in configs:
        savings = calculate_savings(naive_cost, cost)
        pct = calculate_savings_percent(naive_cost, cost) if naive_cost > 0 else 0.0
    ...
    print(f"Monthly Savings:         ${calculate_savings(naive_cost, best_overall_cost):.2f}")
    print(f"Percentage Saved:        {calculate_savings_percent(naive_cost, best_overall_cost):.1f}%\n")
...
def print_final_recommendation():
    ...
    # GPT-4.1 Nano + Prompt B (Optimized) + Prompt Caching
    cached_input_cost = ((p_b["input_tokens"] * 0.8 * 0.5 * pricing["input_per_1m"] + p_b["input_tokens"] * 0.2 * pricing["input_per_1m"]) / 1_000_000) * MONTHLY_TICKETS
    out_cost = (p_b["output_tokens"] / 1_000_000) * pricing["output_per_1m"] * MONTHLY_TICKETS
    realtime_cost = cached_input_cost + out_cost
    realtime_savings_pct = calculate_savings_percent(naive_cost, realtime_cost)

    # Async / Overnight Batch Processing (Prompt B + GPT-4.1 Nano + Prompt Caching + Batch API)
    batch_cost = realtime_cost * 0.50
    batch_savings_pct = calculate_savings_percent(naive_cost, batch_cost)
    ...
    print(f"   - Monthly Cost: ${realtime_cost:.2f}/month ({realtime_savings_pct:.1f}% savings over Naive)")
    ...
    print(f"   - Monthly Cost: ${batch_cost:.2f}/month ({batch_savings_pct:.1f}% savings over Naive)")
