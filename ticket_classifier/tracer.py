...
try:
    ...
    logger.error("Failed to set up langsmith tracing: %s", exc)
    _record_trace = None
...
def flush_langfuse_traces():
    ...
    if langfuse_client is not None:
        try:
            ...
            logger.error("Error flushing Langfuse traces: %s", exc)
...
def trace_llm_call(
    ...
    try:
        ...
        logger.error("Error recording LangSmith trace: %s", exc)
