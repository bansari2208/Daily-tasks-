...
def generate_report(log_file=None, batch_time: float = None):
    """Reads JSON logs and prints the report summary using pathlib and resilient JSON Lines reading."""
    log_path = Path(log_file).resolve() if log_file else DEFAULT_LOG_FILE

    # Check if the log file exists
    if not log_path.exists():
        print(
            f"No log file found at '{log_path}'. Run LLM requests first to generate logs."
        )
        return

    # Read and parse JSON log entries line by line, skipping invalid JSON lines
    logs = []
